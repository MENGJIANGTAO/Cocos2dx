...
{
	// add close menu
	auto closeItem = MenuItemImage::create(s_pathClose, s_pathClose, CC_CALLBACK_1(controller::closeCallback, this));
	...
	for (int i = 0; i < g_testCount; ++i)
	{
		...
		auto menuItem = MenuItemLabel::create(label, CC_CALLBACK_1(controller::menuCallback, this));
	...
	mouseListener->onMouseScroll = CC_CALLBACK_1(controller::onMouseScroll, this);
...
void controller::menuCallback(Ref * sender)
{
...
void controller::closeCallback(Ref * sender)
{
...
void controller::onMouseScroll(Event *event)
{
...
void controller::runAllTests(int fd)
{
	...
	for (int i = 0; i < g_testCount; i++)
	{
		...
		wait(1);
		BaseGo* firstTest = app->getCurrentGo();
		...
		while (1)
		{
			...
			wait(1);
			BaseGo * curTest = app->getCurrentGo();
			if (curTest == nullptr)
			{
				break;
			}
...
void controller::addConsoleAutoTest()
{
	...
	static struct Console::Command autotest = {
		...
		[this](int fd, const std::string& args)
		{
			...
			AppDelegate* app = (AppDelegate *)Application::getInstance();
			BaseGo* currentTest = app->getCurrentGo();
			if (args == "next")
			{
			...
			{
				_exitThread = false;
				std::thread t = std::thread(&controller::runAllTests, this, fd);
				t.detach();
				return;
			}
...
void controller::startAutoRun()
{

	std::thread t = std::thread(&controller::autorun, this);
	t.detach();
}

ssize_t controller::readline(int fd, char* ptr, size_t maxlen)
{
	size_t n, rc;
	char c;
