	...
	if (_drawing) {
		...
		if (touch) {
			...
			if (dx * dx + dy * dy > pow(_minLineLength, 2)) {
				...
				_lineContainer->set_LineType(LINE_TEMP);
			}
			else {
...
void GameLayer::ccTouchesEnded(CCSet* pTouches, CCEvent* event) {
	...
	if (touch) {
		...
		if (_lineContainer->get_LineType() == LINE_TEMP) {
			//set up dashed line
			_lineContainer->setPivot(tap);
			...
			_lineContainer->set_LineType(LINE_DASHED);
		}
